...
| 结果校验 | 逐段生成并统一排版；检查引用、主体、镜头时间和中文原文；失败自动修正一次 |
...
节点逐段优化，由程序固定三段或六段标题及顺序。遇到引用、主体或原文校验失败，会自动修正一次，仍失败则停止，不将无效结果送入 H3。对于从 0 秒开始、连续且不超时的 `0-3秒…；3-6秒…` 分镜，程序保留切镜时间，并将明确标注的 `台词“…”`、`字幕“…”` 原样放回对应镜头。其他时间写法仍由模型编排。

Skill 源文件完整随包保留；推理时按当前模式和字段选取规范章节，排除完整案例及代码块示例，减少模板串用。`max_new_tokens` 是单次生成上限，逐段/逐镜生成可能比旧版耗时更长。

先减少参考素材数量和需求长度。显存不足可降低图像分析尺寸或视频采样帧数；输出达到 token 上限时可适当提高 `max_new_tokens`。格式或引用校验失败时，检查需求后修改 `refresh` 重新优化。

## 验证状态

2026-09-14：5090 专用环境的真实 AWQ 推理已验证。三张参考图的 15 秒任务通过六段格式、引用、时间及原文校验；无参考素材任务通过三段校验。分段优化期间峰值 CUDA allocated 约 8.35 GiB（这两组输入，非整套 H3 工作流显存）。格式通过不代表语义完全准确，7B 模型仍可能改变背景、动作或声音细节，生成视频前应预览。
...
| 子进程离线标志和取消清理 | 已有离线测试 |
| 示例工作流连线与加载依赖 | 已有离线测试 |
| 真实 AWQ 模型加载与推理 | 5090 上三图任务、无参考任务通过 |
| RunningHub 安装与完整 H3 视频生成 | 未实机验证 |
| 24GB 峰值显存 | 本次优化样本约 8.35 GiB CUDA allocated；不含 H3 生成 |
...
连续中文分镜的逐镜优化现传入分镜前的全局要求、主体定义及适用规则，并恢复可能回显的原文占位符。27 项测试通过，5090 三图真实推理通过格式与原文校验。7B 模型仍可能误写背景、动作和参考关系，不能将该修复理解为语义一致性保证。

画质排查：同一提示词、种子、544×960 分辨率下，将一采前 8/12 步改为完整 12 步后，产品与手部重影明显改善。原 Demo 的二采组被绕过时，一采只是中间预览。此仓库原示例仍保留分段流程；完整单采应直连调度器全部 sigmas，不能再直接叠加原二采。
